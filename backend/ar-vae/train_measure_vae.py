...
# Force CPU before torch sees GPU (for --cpu flag; must run before import torch)
if '--cpu' in sys.argv:
...
@click.option('--cpu', is_flag=True,
              help='force CPU (use if GPU gives cublas/cuDNN errors, e.g. RTX 40xx with PyTorch 1.0)')
def main(
        dataset_type,
        note_embedding_dim,
