...
**Using a modern GPU (e.g. RTX 4060):**  
The conda environment uses PyTorch 1.0, which does not support newer GPUs and can cause `cuDNN`/`cublas` errors. To train on GPU in a few minutes instead of days on CPU, install a current PyTorch build with CUDA in your environment, then run without `--cpu`:

    pip install torch torchvision --index-url https://download.pytorch.org/whl/cu121

Use `cu118` instead of `cu121` if your system has CUDA 11.8. Then run training as usual (omit `--cpu`).
