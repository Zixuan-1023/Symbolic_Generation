...
python -c "import torch; assert torch.cuda.is_available(), 'CUDA not available. Install PyTorch with CUDA: conda install pytorch cudatoolkit=11.3 -c pytorch'"
echo "Using GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo 'GPU 0')"
