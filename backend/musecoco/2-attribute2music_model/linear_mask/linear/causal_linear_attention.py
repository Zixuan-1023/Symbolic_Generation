...
def _causal_dot_product_pytorch_fallback(Q, K, V):
    """Pure PyTorch causal dot product when fast_transformers CUDA kernel is unavailable.
    Q, K, V: [N, H, L, D] - computes out[l] = sum_{j<=l} (Q[l].*K[j]) * V[j]
    """
    # scores[n,h,l,j] = sum_d Q[n,h,l,d]*K[n,h,j,d]
    scores = torch.einsum("nhld,nhjd->nhlj", Q, K)
    L = Q.size(2)
    causal_mask = torch.tril(torch.ones(L, L, device=Q.device, dtype=Q.dtype))
    scores = scores * causal_mask
    # out[n,h,l,d] = sum_j scores[n,h,l,j] * V[n,h,j,d]
    out = torch.einsum("nhlj,nhjd->nhld", scores, V)
    return out
...
def causal_linear(Q, K, V):
    ...
    try:
        ...
        if "'NoneType' object is not callable" in str(e):
            # fast_transformers CUDA kernel not built; use PyTorch fallback on same device
            V_new = _causal_dot_product_pytorch_fallback(Q, K, V)
        else:
            raise
