...
def attributes(args):
    ...
    # Single RNG source: fairseq / sampling must see the same seed as torch/np/python.
    # Previously, ``no_seed_provided`` branched to secrets.randbelow() while ``--seed`` still
    # went to the generator → identical runs under same CLI seed and confusing behavior.
    # - If ``--seed`` is set: use it (backend passes a fresh int per job unless ``render.seed`` pins).
    # - Else: draw once, assign to ``args.seed``, then seed everything.
    seed_arg = getattr(args, "seed", None)
    if seed_arg is None:
        seed_arg = secrets.randbelow(1 << 31)
        args.seed = seed_arg
    seed_everything(int(seed_arg))
    ...
    continuation_text = ""
    continuation_len = 0
    crp = getattr(args, "continuation_remi_path", None)
    if crp and os.path.isfile(crp):
        with open(crp, encoding="utf-8") as _cf:
            continuation_text = _cf.read().strip()
        continuation_len = len(continuation_text.split()) if continuation_text else 0
        print(f"Continue mode: {continuation_len} REMI prefix tokens from {crp}")
    ...
    for batch_step in range(steps + 1):
        ...
        for attribute_prefix in infer_command_token:
            if continuation_text:
                start_tokens.append(
                    " ".join(attribute_prefix) + " <sep> " + continuation_text
                )
            else:
            ...
            for id_, src_tokens, hypos, info in sorted(results, key=lambda x: x[0]):
                ...
                for hypo in hypos[: min(len(hypos), args.nbest)]:
                    ...
                    with open(save_root + f"/{infer_list[id_][1]}/remi/{save_id}.txt", "w") as f:
                        f.write(hypo_str)
                    gen_after_sep = hypo_str.split(" ")[sep_pos[id_] + 1:]
                    if continuation_len > 0:
                        ct = continuation_text.split()
                        if (
                            len(gen_after_sep) >= continuation_len
                            and gen_after_sep[:continuation_len] == ct
                        ):
                            remi_token = gen_after_sep
                        else:
                            remi_token = ct + gen_after_sep
                    else:
                        remi_token = gen_after_sep
                    ...
                    try:
                        ...
                        print(f"MIDI decode failed (save_id={save_id}): {midi_err}. Run remi_to_midi.py on the .txt to convert manually.")
...
def cli_main():
    ...
    parser.add_argument(
        "--continuation_remi_path",
        type=str,
        default=None,
        help="File with space-separated REMI tokens; prefix [attr] <sep> [tokens].",
    )
